...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	  ...
	  !! Modified on 03/02/2020 by Vanshu [Modification #13]
	  !! Modified on 13/01/2020 by Vanshu [Modification #6]
	  real :: alpha
	  real :: x_temporary
	  logical :: fexist
	  !! End [Modification #6]
...
!  -Correcting 2nd order derivatives in non-uniform Grid DIRECTIONS (Updated 2020-11-12)
!  -Nomenclature: (old) g3rm -> dx3c
      !do kc=1,nxm
      !  xm(kc)=(xc(kc)+xc(kc+1))*0.5d0
      !  dx3c(kc)=(xc(kc+1)-xc(kc))*dx ! ModR10: Nomenclature g3rm->dx3c
      !enddo
      !do kc=2,nxm
...
!!
      !do kc=1,nxm
      !  udx3m(kc) = dx/dx3c(kc) ! ModR10: Nomenclature g3rm->dx3c
      !  udx3c(kc) = dx/g3rc(kc)
      !end do
      !udx3c(nx) = dx/g3rc(nx)
...
!        kp=kc+1
!        a33=dxq/g3rc(kc) 
!        a33p=1.d0/dx3c(kc) ! ModR10: Nomenclature g3rm->dx3c
!        a33m=1.d0/dx3c(km) ! ModR10: Nomenclature g3rm->dx3c
!        ap3ck(kc)=a33*a33p
!        am3ck(kc)=a33*a33m
...
!       kp=kc+1
!       km=kc-1
!       a33=dxq/dx3c(kc) ! ModR10: Nomenclature g3rm->dx3c
!       a33p= +a33/g3rc(kp)
!       a33m= +a33/g3rc(kc)
!       ap3sk(kc)=a33p
!       am3sk(kc)=a33m
...
!       a33m= +a33/g3rc(kc)
!       ap3sk(kc)=a33p
! 	  !! Modified on 17/01/2020 by Vanshu after discussion with ChongShen Ng [Modification #4]
!       am3sk(kc)=0.d0
! 	  !! End [Modification #4]
