...
subroutine SolveImpEqnUpdate_Z
    ...
    use param
    use local_arrays, only : vz,rhsx,rhsy,rhsz
    use ibm_arrays, only: ibm_gz_px,ibm_gz_py,ibm_gz_pz
    ...
    do jc=zstart(2),zend(2)
        do kc=zstart(1),zend(1)
            do ic=1,nzm
                acil_b   = 1.0d0/(1.0d0+2.0d0*dzq*betadx)
                if ((ibm_gz_pz(kc,jc,ic)).or.(ic.eq.1)) then
                    amil(ic) = 0.0d0
                    acil(ic) = 1.0d0
                    apil(ic) = 0.0d0
                    rz1d(ic) = rhsz(kc,jc,ic)
                else
                    amil(ic) = -dzq*betadx*acil_b
                    acil(ic) = 1.0d0
                    apil(ic) = -dzq*betadx*acil_b
    ...
    do ic=ystart(3),yend(3)
        do kc=ystart(1),yend(1)
            do jc=1,nym
                acjl_b = 1.0d0/(1.0d0+2.0d0*dyq*betadx)
                if ((ibm_gz_py(kc,jc,ic)).or.(ic.eq.1)) then
                    amjl(jc) = 0.0d0
                    acjl(jc) = 1.0d0
                    apjl(jc) = 0.0d0
                    ry1d(jc) = rhsy(kc,jc,ic)
                else
                    amjl(jc) = -dyq*betadx*acjl_b
                    acjl(jc) = 1.0d0
                    apjl(jc) = -dyq*betadx*acjl_b
    ...
    do ic=xstart(3),xend(3)
        do jc=xstart(2),xend(2)
            do kc=1,nxm
                ...
                if ((ibm_gz_px(kc,jc,ic)).or.(ic.eq.1)) then
                    amkl(kc) = 0.0d0
                    ackl(kc) = 1.0d0
                    apkl(kc) = 0.0d0
    ...
    do ic=xstart(3),xend(3)
        do jc=xstart(2),xend(2)
            do kc=1,nxm
                vz(kc,jc,ic) = vz(kc,jc,ic) + rhsx(kc,jc,ic)
            end do
        end do
    end do
