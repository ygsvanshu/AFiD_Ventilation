...
subroutine SolveImpEqnUpdate_X
    ...
    use param
    use local_arrays, only : vx,rhsx,rhsy,rhsz
    ...
    do jc=zstart(2),zend(2)
        do kc=zstart(1),zend(1)
            do ic=1,nzm
                ...
                if (ibm_gx_pz(kc,jc,ic).or.(kc.eq.1).or.(kc.eq.nx)) then
                    amil(ic) = 0.0d0
                    acil(ic) = 1.0d0
                    apil(ic) = 0.0d0
    ...
    do ic=ystart(3),yend(3)
        do kc=max(2,ystart(1)),min(nxm,yend(1))
            do jc=1,nym
                acjl_b   = 1.0d0/(1.0d0-ac2gj*betadx)
                if (ibm_gx_py(kc,jc,ic).or.(kc.eq.1).or.(kc.eq.nx)) then
                    amjl(jc) = 0.0d0
                    acjl(jc) = 1.0d0
                    apjl(jc) = 0.0d0
    ...
    do ic=xstart(3),xend(3)
        do jc=xstart(2),xend(2)
            do kc=1,nx
                ...
                if (ibm_gx_px(kc,jc,ic).or.(kc.eq.1).or.(kc.eq.nx)) then
                    amkl(kc) = 0.0d0
                    ackl(kc) = 1.0d0
                    apkl(kc) = 0.0d0
    ...
    do ic=xstart(3),xend(3)
        do jc=xstart(2),xend(2)
            do kc=1,nx
                vx(kc,jc,ic) = vx(kc,jc,ic) + rhsx(kc,jc,ic)
            end do
        end do
    end do
